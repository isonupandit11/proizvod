...
function showproducts(res) {
    for (var i = 0; i < res.length; i++) {
        ...
        row += '<button  onclick="addtocart(\'' + res[i].image + '\',\'' + res[i].title + '\',\'' + res[i].price + '\',\'' + res[i].id + '\')" class="inline-flex ml-40 items-center relative bg-green-800 border-0 py-1 px-3 focus:outline-none hover:bg-blue-700 rounded text-white mt-4 md:mt-0">Add To Cart</Button>';
        row += '</div>'
        row += '</div>';
        row += '</div>';
...
function addtocart(image, titile, price, id) {
    $.ajax({
        ...
        data: "{ image:'" + image + "',titile:'" + titile + "',price:'" + price + "',id:'" + id + "'}",
